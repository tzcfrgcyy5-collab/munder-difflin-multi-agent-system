...
def inventory_assessment_tool(
    ...
    """
    return json.dumps(_inventory_assessment(json.loads(items_json), request_date, deadline))
...
def _execute_framework_tool(tool_object, **kwargs) -> str:
    """Execute a smolagents tool while remaining compatible with simple test doubles."""
...
class PaperCompanyOrchestratorAgent:
    ...
    def run(
        ...
        if unsupported or not items:
            missing = ", ".join(unsupported) if unsupported else "the requested products"
            return (
                "ORDER NOT FULFILLED. We could not match the following item(s) to our "
                f"current paper-products catalog: {missing}. No stock or payment transaction "
                "was recorded. Please request an available substitute or a revised order."
            )
        ...
        if not inventory_plan["feasible"]:
            return (
                ...
                + ". No sale was recorded. Please provide a later delivery date or reduce the quantity."
            )
        ...
        if not sale["fulfilled"]:
            return (
                "ORDER NOT FULFILLED. Final inventory verification was unsuccessful, so no "
                "sale was completed. Please contact us for an updated availability check."
            )
        ...
        history_text = (
            f" Pricing was checked against {quote['historical_quotes_consulted']} similar "
            "historical quote(s)."
            if quote["historical_quotes_consulted"]
            else " Catalog pricing was used because no close historical quote was available."
        )
        return (
            f"ORDER FULFILLED. Items: {item_summary}. Quote total: ${quote['total']:.2f}."
